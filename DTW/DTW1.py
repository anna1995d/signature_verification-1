...
def DTW (A, B, n, w, dist):
	...
	for i in range(1, min(w,max(na,nb))):
		if(i<na):
			D[i][0]=D[i-1][0] + dist(A[i],B[0],n)
		if(i<nb):
			D[0][i]=D[0][i-1] + dist(A[0],B[i],n)

	
	for i in range(1, na):
		for j in range(max(1, i-w+1), min(nb, i+w)):
			D[i][j] = min(D[i-1][j-1], D[i][j-1], D[i-1][j])  + dist(A[i], B[j], n)
...
# DTW([0,1,1,2,3,2,1],[1,1,2,3,2,0],1,10,euclidean)
# c = [[-.6,-.46],[-.65,-.62],[-.71,-.68],
