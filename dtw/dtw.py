...
class DTW(object):
    ...
    def calculate(self):
        ...
        ans[0][0] = dis[0][0] = self.dist_func(self.first_vec[0], self.second_vec[0])[0]
        for i in range(1, min(self.window_size, self.first_vec.shape[0])):
            if dis[i][0] == -1:
                dis[i][0] = self.dist_func(self.first_vec[i], self.second_vec[0])[0]
            ans[i][0] = ans[i - 1][0] + dis[i][0]

        for i in range(1, min(self.window_size, self.second_vec.shape[0])):
            if dis[0][i] == -1:
                dis[0][i] = self.dist_func(self.first_vec[0], self.second_vec[i])[0]
        ...
        for i in range(1, self.first_vec.shape[0]):
            for j in range(max(1, i - self.window_size + 1), min(self.second_vec.shape[0], i + self.window_size)):
                if dis[i][j] == -1:
                    dis[i][j] = self.dist_func(self.first_vec[i], self.second_vec[j])[0]
