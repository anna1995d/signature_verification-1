...
def get_autoencoder_train_data(usr_num):
    (gen_x, gen_y) = DATA.get_genuine_combinations(usr_num, CONFIG.ae_smp_cnt)
    return sequence.pad_sequences(gen_x, value=CONFIG.msk_val, maxlen=DATA.gen_max_len), \
        sequence.pad_sequences(gen_y, value=CONFIG.msk_val, maxlen=DATA.gen_max_len), \
